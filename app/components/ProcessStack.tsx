...
export default function ProcessStack() {
  ...
  useEffect(() => {
    ...
    const update = (p: number) => {
      // 1. FIXED PACING MATH: Spreads the card animations evenly across 100% of the scroll track
      const windows = STEPS.map((_, i) => {
        const stepSize = 1 / (n - 0.2);
        const start = i * stepSize * 0.75;
        const end = Math.min(1, start + stepSize * 1.1);
    ...
    const tween = gsap.to(dummy, {
      ...
      scrollTrigger: {
        ...
        scrub: 0.4,
        // 2. FIXED EARLY TRIGGER: Forces GSAP to recalculate positions correctly when layout shifts
        invalidateOnRefresh: true, 
        onUpdate: (self) => update(self.progress),
      },
    });

    // 3. FIXED LAYOUT SYNC: Wait for images/fonts above to render before confirming trigger marks
            ...
            {STEPS.map((step, i) => (
              <div key={step.num} className="rounded-2xl bg-white border border-black/10 overflow-hidden flex flex-col shadow-lg">
                <div className="relative h-40 shrink-0 bg-gray-100">
                  ...
                  <div className="absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center text-sm font-black shadow-sm" style={{ backgroundColor: ACCENTS[i], color: CHARCOAL }}>
                    {step.num}
                  </div>
                </div>
                <div className="p-8">
  ...
  return (
    <section style={{ backgroundColor: WHITE }} className="relative z-40 border-t border-black/10">
      ...
      {/* 4. FIXED HEIGHT: Reduced from 540vh to 300vh so the 5 cards scrub naturally without dead scroll space */}
        ...
        <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
          <div className="relative w-full max-w-md px-4 md:px-0 h-[580px] sm:h-[500px] md:h-[540px]" style={{ perspective: '1400px' }}>
            {STEPS.map((step, i) => (
              ...
              >
                <div className="relative h-32 sm:h-36 md:h-40 shrink-0 bg-gray-100">
                  <img src={step.img} alt={step.title} className="absolute inset-0 w-full h-full object-cover" />
                  <div
                    className="absolute top-4 right-4 w-10 h-10 rounded-full flex items-center justify-center text-sm font-black shadow-sm"
